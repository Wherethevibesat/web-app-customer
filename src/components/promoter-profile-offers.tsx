...
export function PromoterProfileOffers({
  offers,
  isSignedIn,
}: {
  ...
  if (offers.length === 0) {
    return <p className="text-sm text-wtva-muted">No active offers right now.</p>;
  }
        ...
        {offers.map((o) => (
          ...
          >
            ...
            <p className="text-sm text-wtva-muted">
              {formatPrice(o.price_cents)} · {o.capacity - o.slots_used} left
            </p>
            {o.description && (
              <p className="mt-1 text-sm text-wtva-muted">{o.description}</p>
            )}
            {o.allow_inquire && (
              <button
                type="button"
